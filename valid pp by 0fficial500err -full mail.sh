...
printf "$GRN            ██████║ ╚██████╔╝ ╚██████╔╝██╗███████╗ ██║  ██╗ ██║  ██╗ \n"
printf "$GRN            ╚═════╝  ╚═════╝   ╚═════╝ ╚═╝╚══════╝ ╚═╝  ╚═╝ ╚═╝  ╚═╝ \n"
printf "$RED   _________________________________________                         \n"
printf "$CYAN  #  Recode BY Slenthonk69                #\n"
printf "$CYAN  #      Power BY:                        #\n"
printf "$CYAN  #      0fficial500err                   #\n"
printf "$CYAN  # - https://github.com/500err0fficial   #\n"
printf "$RED   _________________________________________\n"
printf "$NC\n"
cat <<EOF
...
ORIGINAL SCRIPT BY: Slenthonk69
"
  exit 1 
}
...
if [[ $inputFile == '' ]]; then
  ...
  # current folder
  # clear
  read -p "Nama Mailist Mu: " inputFile
fi

if [[ $targetFolder == '' ]]; then
  read -p "Nama Folder Result: " targetFolder
  ...
  if [[ ! -d "$targetFolder" ]]; then
    ...
    read -p "$targetFolder/ folder sudah ada, timpa ? [y/n]: " isAppend
    if [[ $isAppend == 'n' ]]; then
      exit
    fi
  fi
...
urlencode() {
  ...
  # posted=`curl "https://www.paypal.com/cgi-bin/webscr" --data "cmd=_cart&upload=1&business=$1&item_name_1=Iyaharuuum&amount_1=12.35&shipping_1=2.45" --compressed -D - -s --cookie-jar drop.cook --cookie asu`
  posted=`curl "https://www.paypal.com/cgi-bin/webscr" --data "item_name=Sunbury$RAND.COM+-+Monthly+Email+Address&p3=1&t3=Y&cmd=_xclick-subscriptions&currency_code=AUD&sra=1&src=1&business=$1" --compressed -D - -s --cookie-jar drop-cookies.txt --cookie cookies.txt`
  duration=$SECONDS
  ...
  footer="[Slenthonk69 - PayPal Valid] $(($duration % 60))sec.\n"
  ...
  if [[ $countryCode == '' ]]; then
    ...
    printf "[$header] $2/$3. ${GRN}LIVE [$country] ($countryCode) => $1 ${NC} $footer"
...
echo "Mail: `grep -c "@mail" $inputFile`"
echo "gmx: `grep -c "@gmx" $inputFile`"
echo "___________________________________________________"
...
if [[ $isCompress == 'y' ]]; then
  ...
  mv $targetFolder 500err0fficial
  echo "[+] $targetFolder has been moved to 500err0fficial/"
fi
#rm $inputFile
duration=$SECONDS
echo "Checking done in $(($duration / 3600)) hours $(($duration / 60 % 60)) minutes and $(($duration % 60)) seconds."
echo "+==========+ Slenthonk69 || - Power by 500err0fficial || Slenthonk69 +==========+"